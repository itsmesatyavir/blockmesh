...
def print_banner():
    banner = f"{Fore.GREEN}{Style.BRIGHT} WARNING:- Tool Shared By FOREST ARMY (https://t.me/forestarmy) NO NEED TO PURCHASE ANY SCRIPT"
    print(banner)
...
def submit_bandwidth(email, api_token, ip_info, proxy_config):
    ...
    try:
        ...
        response.raise_for_status()
        print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{ Fore.GREEN} Up time for {ip_info.get('ip')}")
    except requests.RequestException as err:
        if err.response:
            status_code = err.response.status_code
            response_text = err.response.text
            print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{Fore.YELLOW} Failed to Up time {email}: {status_code} - {response_text}")
        else:  
            print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{Fore.RED} Failed to Up time {email}: {err}")
...
def get_and_submit_task(email, api_token, ip_info, proxy_config):
    ...
    try:
        ...
        print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{Fore.GREEN} Got task success!: {task_id}")
...
def format_proxy(proxy_string):
    ...
    if "@" in address:
        ...
        proxy_dict = {
            "http": f"{proxy_type}://{username}:{password}@{host_port}",
            "https": f"{proxy_type}://{username}:{password}@{host_port}"
        }
    else:
...
def authenticate(proxy, email, password):
    ...
    try:
        response = requests.post(
            login_endpoint, json=login_data, headers=login_headers, proxies=proxy_config)
        response.raise_for_status()
        auth_data = response.json()
        api_token = auth_data.get("api_token")
        proxy_tokens[email] = api_token  # Store the token by email
        ip = proxy_config["http"].split("@")[1].split(":")[0]
        print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{Fore.GREEN} Login successful for {email} IP: {ip}")
        return api_token
    except requests.RequestException as err:
        if err.response:
            status_code = err.response.status_code
            response_text = err.response.text
            print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{Fore.YELLOW} Login failed for {email} using {ip}: {status_code} - {response_text}")
        else:  
            print(f"{Fore.LIGHTCYAN_EX}[{datetime.now().strftime('%H:%M:%S')}]{Fore.RED} Login failed for {email} using {ip}: {err}")
        return None


def process_proxy_with_credentials(proxy, credentials):
    ...
    if not api_token:  # Authenticate if no token is found
        api_token = authenticate(proxy, email_input, password_input)

    while True:
        if api_token:
            ip_info = get_ip_info(format_proxy(proxy)["http"].split("@")[-1])
            submit_bandwidth(email_input, api_token,
                             ip_info, format_proxy(proxy))
            time.sleep(random.randint(60, 120))
            get_and_submit_task(email_input, api_token,
                                ip_info, format_proxy(proxy))
            time.sleep(random.randint(60, 120))
...
def main():
    ...
    if len(credentials_list) > len(proxies_list):
        print(f"{Fore.RED}[×] Số lượng tài khoản và proxy không bằng nhau!{Style.RESET_ALL}")
        exit()
    threads = []
    for proxy, credentials in zip(proxies_list, credentials_list):
        thread = threading.Thread(
            target=process_proxy_with_credentials, args=(proxy, credentials))
        thread.daemon = True
